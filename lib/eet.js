...
function getSoapClient(soapOptions) {
  let key = soapOptions.endpoint ? soapOptions.endpoint : "default"

  if (soapClients.has(key)) {
    console.log("reusing soap client")
    return Promise.resolve(soapClients.get(key))
  } else {
    console.log("creating new soap client")
    return new Promise((resolve, reject) => {
      soap.createClient(WSDL, soapOptions, (err, client) => {
        if (err) return reject(err)
        soapClients.set(key, client)
...
function doRequest (options, items) {
  ...
  if (options.request) {
    soapOptions.request = options.request
  }
  const timeout = options.timeout || 2000
  const offline = options.offline || false

  console.log("doRequest start", new Date().getTime() - options.startTime)
  return getSoapClient(soapOptions).then(client => new Promise((resolve, reject) => {
    console.log("doRequest createClient", new Date().getTime() - options.startTime)
    ...
    client.setSecurity(new WSSecurity(options.privateKey, options.certificate, uid))
      client.OdeslaniTrzby(body, (err, response) => {
        console.log("doRequest response OdeslaniTrzby", new Date().getTime() - options.startTime)
        if (err) return reject(err)
        try {
          validate.httpResponse(response)
          console.log("doRequest validated", new Date().getTime() - options.startTime)
          resolve(getResponseItems(response))
        } catch (e) {
          console.log("doRequest validated err", new Date().getTime() - options.startTime)
          reject(e)
        }
      }, {timeout: timeout})
  })).catch(err => {
